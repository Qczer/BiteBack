...
export const ensureCorrectUser = (req, res, next) => {
    if (!req.user)
        return res.status(500).json({ message: "Błąd serwera: brak danych użytkownika" });

    if (req.user._id === req.params.userID)
        next();
    else
        res.status(403).json({ message: "Brak uprawnień do zasobów tego użytkownika" });
};